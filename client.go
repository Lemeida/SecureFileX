...
func sendContent(sendWindow, receiveWindow fyne.Window) fyne.CanvasObject {
	...
	optionBtn := widget.NewButtonWithIcon("Option", theme.SettingsIcon(), func() {
		send_port = showOptionWindow(sendWindow)
	})
	...
	sendBtn := widget.NewButtonWithIcon("Send", theme.MailSendIcon(), func() {
		client1(selectedFilePath, send_addressEditor.Text, send_port1.Text, send_port, U1_H, ppk_1)
	})
...
func receiveContent(sendWindow, receiveWindow fyne.Window) fyne.CanvasObject {
	...
	optionBtn := widget.NewButtonWithIcon("Option", theme.SettingsIcon(), func() {
		recv_port = showOptionWindow(receiveWindow)
	})
	optionBtn.Importance = widget.DangerImportance
	...
	startBtn := widget.NewButtonWithIcon("Listen", theme.MediaPlayIcon(), func() {
		client2(selectedDirPath, recv_addressEditor.Text, recv_port1.Text, U2_H, recv_port)
	})
...
func showOptionWindow(parent fyne.Window) string {
...
func client1(selectedFilePath string, receiver_IP string, receiver_port string, send_port string, U1_H *Collosopn_Resistant_Hash, ppk_1 *PPK_1) {
	pk_1, _, SigC := c_init()
	sys := setup_client()
	...
	send_c(file_content, U1_H, U2_H, sys, pk_2, ppk_1)

	message := joinParams(pk_1.Q.String(), pk_1.R.String(), string(SigC.c1), SigC.c2.String(), SigC.U.String(), SigC.v.String(), U1_H.Hash1.String(), U1_H.Hash2.String(), U1_H.Hash3.String(), U1_H.Hash4.String())
...
func send_c(data []byte, U1_H *Collosopn_Resistant_Hash, U2_H *Collosopn_Resistant_Hash, sys *SystemSetup, pk_2 *PublicKey_2, ppk_1 *PPK_1) (*PublicKey_1, *SecretKey_1, *Signcryption) {
	...
	SigC, U1_H.Hash3, U1_H.Hash4 = sys.Signcryption(data, U1_H, U2_H, sk_1, pk_2) //文件的加密在这个函数中，注意在加密之前对文件进行处理。
...
func (sys *SystemSetup) PPKGenerationUser1() (*PPK_1, *big.Int) {
	...
	ppk_1.d = tmpBig
	println("ppk_1.R:")
	println(len(ppk_1.R.Bytes()))
	println(ppk_1.R.Bytes())
	println("ppk_1.d:")
	println(len(ppk_1.d.Bytes()))
	println(ppk_1.d.Bytes())
	println(base64.StdEncoding.EncodeToString(append(ppk_1.R.Bytes(), ppk_1.d.Bytes()...)))
	//test
...
// 假设客户端1签密，部署在客户端1上
func (sys *SystemSetup) Signcryption(data []byte, U1_H *Collosopn_Resistant_Hash, U2_H *Collosopn_Resistant_Hash, sk_1 *SecretKey_1, pk_2 *PublicKey_2) (*Signcryption, *big.Int, *big.Int) {
...
func client2(selectedDirPath string, sender_IP string, senderPort string, U2_H *Collosopn_Resistant_Hash, receiver_port string) {
	pk_2, _ := r_init()
	sys := setup_client()
	var params []string

	message := joinParams(U2_H.Hash1.String(), U2_H.Hash2.String(), U2_H.Hash3.String(), U2_H.Hash4.String(), pk_2.Q.String(), pk_2.R.String())
	...
	//_, UnSigC := recv_u(U1_H, U2_H, left, right, sys, SigC, pk_1, ppk_2)
...
func (sys *SystemSetup) PPKGenerationUser2() (*PPK_2, *big.Int) {
	...
	ppk_2.d = tmpBig
	println("ppk_2.R:")
	println(len(ppk_2.R.Bytes()))
	println(ppk_2.R.Bytes())
	println("ppk_2.d:")
	println(len(ppk_2.d.Bytes()))
	println(ppk_2.d.Bytes())
	println(base64.StdEncoding.EncodeToString(append(ppk_2.R.Bytes(), ppk_2.d.Bytes()...)))
...
//func recv_u(U1_H *Collosopn_Resistant_Hash, U2_H *Collosopn_Resistant_Hash, left *big.Int, right *big.Int, sys *SystemSetup, SigC *Signcryption, pk_1 *PublicKey_1) (*PublicKey_2, string) {
//	pk_2, sk_2 := r_init()
//
//	//todo equal 1
//	left = sys.PointMul(ppk_2.d, sys.P)
